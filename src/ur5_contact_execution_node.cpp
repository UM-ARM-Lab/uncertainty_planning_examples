...
inline ur5_linked_common_config::SLC ExtractConfig(const std::vector<std::string>& reference_joint_names, const std::vector<std::string>& current_joint_names, const std::vector<double>& current_joint_positions, const ur5_linked_common_config::SLC& reference_config)
{
    ...
    ur5_linked_common_config::SLC current_config;
    ...
    for (size_t idx = 0; idx < reference_joint_names.size(); idx++)
    {
        ...
        const ur5_linked_common_config::SJM& reference_joint = reference_config[idx];
...
inline std::vector<ur5_linked_common_config::SLC, std::allocator<ur5_linked_common_config::SLC>> move_robot(const std::vector<std::string>& joint_names, const ur5_linked_common_config::SLC& target_config, const ur5_linked_common_config::SLC& expected_result_config, const double duration, const double execution_shortcut_distance, const bool reset, ros::ServiceClient& robot_control_service)
{
    ...
    if (reset)
    {
        ...
        {
            const ur5_linked_common_config::SJM& current_joint = target_config[idx];
    ...
    else
    {
        ...
        {
            const ur5_linked_common_config::SJM& current_joint = target_config[idx];
            const ur5_linked_common_config::SJM& expected_joint = expected_result_config[idx];
    ...
    std::vector<ur5_linked_common_config::SLC, std::allocator<ur5_linked_common_config::SLC>> configs(trajectory.size());
...
{
    ros::init(argc, argv, "ur5_contact_execution_node");
    ros::NodeHandle nh;
    ...
    ros::ServiceClient set_uncertainty_service = nh.serviceClient<uncertainty_planning_examples::SetLinkedActuationError>("ur5_robot/set_actuation_uncertainty");
